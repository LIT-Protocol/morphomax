...
class Node {
  ...
  draw(ctx: CanvasRenderingContext2D, time: number, isActive: boolean) {
    ...
    if (isActive) {
      if (this.type === 'heaven') {
        ctx.fillStyle = `rgba(255, 66, 5, ${this.opacity})`;
        ctx.shadowBlur = 2;
        ctx.shadowColor = 'rgba(255, 66, 5, 0.3)';
      } else if (this.type === 'earth') {
        ctx.fillStyle = `rgba(255, 66, 5, ${this.opacity * 0.7})`;
        ctx.shadowBlur = 1;
        ctx.shadowColor = 'rgba(255, 66, 5, 0.2)';
      } else {
        ctx.fillStyle = `rgba(255, 66, 5, ${this.opacity + 0.2})`;
        ctx.shadowBlur = 3;
        ctx.shadowColor = 'rgba(255, 66, 5, 0.4)';
      }
    } else {
      if (this.type === 'heaven') {
...
const TetheredFlow: React.FC<TetheredFlowProps> = ({ isActive, className = '' }) => {
    ...
    const drawConnections = () => {
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (const conn of node.connections) {
          ...
          ctx.quadraticCurveTo(midX + flowOffsetX, midY + flowOffsetY, targetNode.x, targetNode.y);

          ctx.strokeStyle = isActive
            ? `rgba(255, 66, 5, ${Math.max(conn.opacity * 0.8, 0.3)})`
